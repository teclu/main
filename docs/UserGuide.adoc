= K.N.S. AddressBook v1.5.0rc - User Guide
:toc:
:toc-title:
:toc-placement!:
:sectnums:
...
If you're ever in need of help, or just curious to learn more about K.N.S you can use : +
`help` +
which will open up a help window that contains the guide you're currently reading!

=== Adding a person: `add` or `a`

To add a person to K.N.S, you can use the command : +
`add` or `a` `n/NAME [p/PHONE_NUMBER] [e/EMAIL] [a/ADDRESS] [b/BIRTHDAY] [v/AVATAR] [t/TAG]...`
...
==== Partial matching
`Since v1.2` +
You don't have to type the full keyword in order to find a desired person, the `find` command accepts partial matches by default. +
...
To remove a person from your address book, you can use : +
...
To sort the current list, you can use : +
`sort [PREFIX] [ORDER]`
...
To view all the commands that you have entered in reverse chronological order, use : +
`history` or `h`
...
[WARNING]
When you import data, you cannot get your old data back once you close the application. (_you can still use `undo` if you haven't_) +
...
To exit the program, you can use the command:  +
`exit`
...
Add Tag, "addtag | at", at INDEX TAG..., addtag 2 friends highSchool
Delete Tag, "deletag | dt", dt INDEX TAG..., deletetag 2 friends highSchool
