...
designed for telemarketers who are bound to their desks, making many calls to clients with the office's telephone, while on the same time opening their PC/laptop to find their clients' contact number.
...
This is where the details of a selected contact will be displayed. As of now, only the name, address, phone number and
tags are displayed here. Planned features for the Person Panel include:

* `For v1.3` Set a contact's avatar
* `For v1.4` Notes/Comments on the contact
