...
|`* * *` |user with a lousy computer |actually run the application |can actually use its functions
...
|`* * *` |careful/paranoid user |back up my contacts' details |restore them in case addressbook dies on me
...
|`* *` |expert user |set command to my preference |easily remember the command
...
|`* *` |normal user |see recently accessed contact |easily find the person's details without searching

|`* *` |normal user |input case-insensitive command |input command easily

|`* *` |normal user |add a picture to contact |remember my contact better

|`* *` |normal user |get suggestion for command correction |input correct command easily after I input wrongly

|`* *` |normal user |save my contact's birthday |remember my contact's birthday

|`* *` |normal user |get reminded of a contact's birthday |wish him/her happy birthday
...
.  A user should only be able to use the supported commands; unsupported commands should not get executed.
.  A user should be able to add, edit and remove a contact with ease.
.  A user should be able to group/tag their contacts if desired.
